...
المدير يحدّد **رقم أحدث نسخة** و**مسار الملف** من لوحة التحكم (مسار على الموقع
أو رابط خارجي)، أو يرفع الملف من اللوحة / يضعه في `apk/` عبر FTP فيقرأ الخادم
الإصدار **من داخل الملف**. ثم تظهر للمستخدمين — الذين نسختهم **أقدم** — بطاقة
تحديث في تبويب «حسابي» بزر تنزيل (نسبة + سرعة) ثم زر تثبيت.
التفاصيل: `../دليل-تحديث-التطبيق.md`.

| المسار | الوصف |
|---|---|
| `GET /api/app-update` | عامة: آخر إصدار منشور + الرابط + بصمة SHA-256 |
| `GET/PUT /api/admin/app-update` | حالة كاملة / حفظ الإصدار و**مسار الملف** والملاحظات والإلزامي والرابط |
...
> 📎 **قاعدة واحدة لا أكثر:** لا توجد نسخ مهجورة بعد اليوم — قٌصِيَ كل ما سواها بقرار صاحب
> المشروع. النسخة المرجعية الوحيدة هي `api/storage/app.sqlite` (٤٠٤ خدمة) — راجع
> `_data-archive/README.md`، وهي محفوظة في تاريخ Git إن احتُيج إليها يوماً.
...
**لا توجد إلا نسخة واحدة** — لا مهجورة ولا مكرّرة.
...
| الفلاتر | ٦ فلاتر · ٨ إسنادات |
| تحديث التطبيق | إعدادات `app_*` مضبوطة — الإصدار 1.2.0 (بناء 3) |

> هذه هي **النسخة نفسها** التي رفعها صاحب المشروع إلى الخادم و«تعمل الآن».
> لا تدمج شيئاً ولا ترفع نسخة أقدم — راجع `_data-archive/README.md`.
